...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    if (buffer) try { (type = await conn.getFile(buffer), buffer = type.data) }
        ...
        let prep = generateWAMessageFromContent(jid, { extendedTextMessage: { text: text, contextInfo: { externalAdReply: { title: title, body: body, thumbnail:
 buffer, sourceUrl: url }, mentionedJid: await conn.parseMention(text) }}}, { quoted: quoted })
        ...
        try {
            ...
            const resolveLidFromCache = async (jid, groupChatId) => {
                ...
                if (global.lidResolver) {
                    ...
                    try {
                        const resolvedJid = await global.lidResolver.resolveLid(
jid, groupChatId, 2);
